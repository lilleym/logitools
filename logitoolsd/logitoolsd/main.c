/*
    This file is part of logitools.

    logitools is free software; you can redistribute it and/or modify
    it under the terms of the GNU General Public License as published by
    the Free Software Foundation; either version 2 of the License, or
    (at your option) any later version.

    logitools is distributed in the hope that it will be useful,
    but WITHOUT ANY WARRANTY; without even the implied warranty of
    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
    GNU General Public License for more details.

    You should have received a copy of the GNU General Public License
    along with logitools; if not, write to the Free Software
    Foundation, Inc., 51 Franklin St, Fifth Floor, Boston, MA  02110-1301  USA

    (c) 2011 Michael Manley; 2006-2007 The G15tools Project - g15tools.sf.net
*/
        ...
        case G15_EVENT_KEYPRESS: {
        	...
        	/* hack - keyboard events are always sent from the foreground even when they aren't 
                ...
                if(value & G15_KEY_M1 && value & G15_KEY_M3) {
                  ...
                  char filename[128];
                  lcd_t *displaying = lcd->masterlist->current->lcd;
                  sprintf(filename,"/tmp/logitoolsd-sc-%i.pbm",scr_num);
...
int main (int argc, char *argv[])
{
    ...
    for (i=0;i<argc;i++) {
        ...
        if (!strncmp(daemonargs, "-v",2) || !strncmp(daemonargs, "--version",9)) {
            ...
            printf("LogiToolsD version %s - %s\n",VERSION,uf_return_running() >= 0 ?"Loaded & Running":"Not Running");
            printf("compiled with liblogitech version %.3f\n\n",lg15ver/1000);
            exit(0);
        }    
    ...
    if(g15daemon_debug){
        ...
        fprintf(stderr, "LogiToolsD CMDLINE ARGS: ");
        for(i=1;i<argc;i++)
          fprintf(stderr, "%s ",argv[i]);
          fprintf(stderr,"\n");
    }
    if(uf_return_running()>=0) {
        g15daemon_log(LOG_ERR,"LogiToolsD already running.. Exiting");
        exit(0);
    }
    ...
    if(uf_create_pidfile() == 0) {
        ...
        uf_conf_open(lcdlist, "/etc/logitoolsd.conf");
        ...
        snprintf((char*)location,1024,"%s/%s",DATADIR,"splash/g15logo2.wbmp");
...
exitnow:
    ...
    uf_conf_write(lcdlist,"/etc/logitoolsd.conf");
